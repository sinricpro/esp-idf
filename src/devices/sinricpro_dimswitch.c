...
static const char *TAG = "sinricpro_dimswitch";
...
static bool dimswitch_request_handler(
    ...
    void *user_data)
{
    sinricpro_dimswitch_device_t *dev = (sinricpro_dimswitch_device_t *)user_data;
...
sinricpro_device_handle_t sinricpro_dimswitch_create(const char *device_id)
{
    ...
    sinricpro_dimswitch_device_t *dev = malloc(sizeof(sinricpro_dimswitch_device_t));
    if (dev == NULL) {
        ESP_LOGE(TAG, "Failed to allocate DimSwitch device");
        return NULL;
    }

    memset(dev, 0, sizeof(sinricpro_dimswitch_device_t));

    strncpy(dev->base.device_id, device_id, sizeof(dev->base.device_id) - 1);
    dev->base.device_type = SINRICPRO_DEVICE_TYPE_DIMSWITCH;
    dev->base.request_handler = dimswitch_request_handler;
    ...
    ESP_LOGI(TAG, "DimSwitch device created: %s", device_id);
...
esp_err_t sinricpro_dimswitch_delete(sinricpro_device_handle_t device)
{
    ...
    sinricpro_dimswitch_device_t *dev = (sinricpro_dimswitch_device_t *)device;
