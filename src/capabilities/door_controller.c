/*
 * Copyright (c) 2019-2025 Sinric. All rights reserved.
 * Licensed under Creative Commons Attribution-Share Alike (CC BY-SA)
 *
 * This file is part of the SinricPro ESP-IDF component
 * (https://github.com/sinricpro/esp-idf)
 */

#include "door_controller.h"
#include "../core/sinricpro_device_internal.h"
#include "../core/sinricpro_event_limiter.h"
#include "sinricpro_types.h"
#include <stdlib.h>
#include <string.h>
#include "esp_log.h"
#include "cJSON.h"

static const char *TAG = "door_ctrl";

struct sinricpro_door_controller {
    sinricpro_door_state_callback_t callback;
    void *user_data;
    sinricpro_event_limiter_handle_t limiter;
};

sinricpro_door_controller_handle_t sinricpro_door_controller_create(void)
{
    sinricpro_door_controller_handle_t handle =
        malloc(sizeof(struct sinricpro_door_controller));

    if (handle == NULL) {
        ESP_LOGE(TAG, "Failed to allocate DoorController");
        return NULL;
    }

    handle->callback = NULL;
    handle->user_data = NULL;
    handle->limiter = sinricpro_event_limiter_create(SINRICPRO_EVENT_LIMIT_STATE);

    if (handle->limiter == NULL) {
        ESP_LOGE(TAG, "Failed to create event limiter");
        free(handle);
        return NULL;
    }

    ESP_LOGD(TAG, "DoorController created");
    return handle;
}

esp_err_t sinricpro_door_controller_set_callback(
    sinricpro_door_controller_handle_t handle,
    sinricpro_door_state_callback_t callback,
    void *user_data)
{
    if (handle == NULL || callback == NULL) {
        return ESP_ERR_INVALID_ARG;
    }

    handle->callback = callback;
    handle->user_data = user_data;
    ESP_LOGD(TAG, "Door state callback registered");
    return ESP_OK;
}

bool sinricpro_door_controller_handle_request(
    sinricpro_door_controller_handle_t handle,
    const char *device_id,
    const char *action,
    cJSON *request_value,
    cJSON *response_value)
{
    if (handle == NULL || action == NULL) {
        return false;
    }

    if (strcmp(action, "setMode") != 0) {
        return false;
    }

    if (handle->callback == NULL) {
        ESP_LOGW(TAG, "No door state callback registered");
        return false;
    }

    /* Extract mode from request ("Close" or "Open") */
    cJSON *mode_item = cJSON_GetObjectItem(request_value, "mode");
    if (mode_item == NULL || !cJSON_IsString(mode_item)) {
        ESP_LOGE(TAG, "Invalid mode in request");
        return false;
    }

    bool state = (strcmp(mode_item->valuestring, "Close") == 0);

    ESP_LOGI(TAG, "Door state request: device=%s, state=%s", device_id,
             state ? "CLOSE" : "OPEN");

    /* Call user callback */
    bool success = handle->callback(device_id, &state, handle->user_data);

    /* Add mode to response */
    const char *response_mode = state ? "Close" : "Open";
    cJSON_AddStringToObject(response_value, "mode", response_mode);

    return success;
}

esp_err_t sinricpro_door_controller_send_event(
    sinricpro_door_controller_handle_t handle,
    const char *device_id,
    bool state,
    const char *cause)
{
    if (handle == NULL || device_id == NULL || cause == NULL) {
        return ESP_ERR_INVALID_ARG;
    }

    if (!sinricpro_event_limiter_check(handle->limiter)) {
        uint32_t wait_ms = sinricpro_event_limiter_time_until_next(handle->limiter);
        ESP_LOGW(TAG, "Door state event rate limited (wait %lu ms)", wait_ms);
        return SINRICPRO_ERR_RATE_LIMITED;
    }

    ESP_LOGI(TAG, "Sending door state event: device=%s, state=%s, cause=%s",
             device_id, state ? "CLOSE" : "OPEN", cause);

    cJSON *value = cJSON_CreateObject();
    cJSON_AddStringToObject(value, "mode", state ? "Close" : "Open");

    esp_err_t ret = sinricpro_core_send_event(device_id, "setMode", cause, value);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to send door state event: %s", esp_err_to_name(ret));
    }

    return ret;
}

void sinricpro_door_controller_destroy(sinricpro_door_controller_handle_t handle)
{
    if (handle == NULL) return;
    if (handle->limiter) sinricpro_event_limiter_destroy(handle->limiter);
    free(handle);
    ESP_LOGD(TAG, "DoorController destroyed");
}
