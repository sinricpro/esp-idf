...
#include "../capabilities/lock_controller.h"
...
static const char *TAG = "sinricpro_lock";
    ...
    void *user_data)
{
    ...
    /* Try lock controller */
    if (sinricpro_lock_controller_handle_request(dev->lock_controller,
                                                   device_id, action,
...
sinricpro_device_handle_t sinricpro_lock_create(const char *device_id)
{
    ...
    sinricpro_lock_device_t *dev = malloc(sizeof(sinricpro_lock_device_t));
    if (dev == NULL) {
        ESP_LOGE(TAG, "Failed to allocate Lock device");
        return NULL;
    }

    memset(dev, 0, sizeof(sinricpro_lock_device_t));

    strncpy(dev->base.device_id, device_id, sizeof(dev->base.device_id) - 1);
    dev->base.device_type = SINRICPRO_DEVICE_TYPE_LOCK;
    dev->base.request_handler = lock_request_handler;
    ...
    ESP_LOGI(TAG, "Lock device created: %s", device_id);
